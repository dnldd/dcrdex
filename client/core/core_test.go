...
func TestDexConnectionOrderBook(t *testing.T) {
	...
	id, err := hex.DecodeString("1d6c8998e93898f872fa43f35ede17c3196c6a1a2054cb8d91f2e184e8ca0316")
	if err != nil {
	...
	id, err = hex.DecodeString("d445ab685f5cf54dfebdaa05232892b4cfa453a566b5e85f62627dd6834c5c02")
	if err != nil {
		t.Fatalf("[DecodeString]: unexpected err: %v", err)
	}
	bookSeq := uint64(2)
	...
	book, ok := dc.books[mid]
	if !ok {
		t.Fatalf("expected order book with market id %s", mid)
	}

	if book.Seq != bookSeq {
		t.Fatalf("expected order book sequence to be %d, got %d", bookSeq, book.Seq)
	}
	...
	if book.Seq != unbookSeq {
		t.Fatalf("expected order book sequence to be %d, got %d", unbookSeq, book.Seq)
	}

	// Ensure handleUnsubOrderBookMsg removes the associated order book as
	// expected.
	msg, err = msgjson.NewRequest(2, msgjson.UnsubOrderBookRoute, &msgjson.UnsubOrderBook{
		MarketID: mid,
	})
	if err != nil {
		t.Fatalf("[NewRequest]: unexpected err: %v", err)
	}

	err = tCore.handleUnsubOrderBookMsg(dc, msg)
	if err != nil {
		t.Fatalf("[handleUnsubOrderBookOrderMsg]: unexpected err: %v", err)
	}

	_, ok = dc.books[mid]
	if ok {
		t.Fatalf("expected order book with market id %s removed", mid)
	}
}
